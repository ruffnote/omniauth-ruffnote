...
describe OmniAuth::Strategies::Ruffnote do  
  ...
  let(:local) do
    OmniAuth::Strategies::Ruffnote.new('GITHUB_KEY', 'GITHUB_SECRET',
      {
        client_options: {
          site: local_site,
  ...
  subject do
    OmniAuth::Strategies::Ruffnote.new({})
  end
  ...
  context "#info" do
    it 'should use name, image from raw_info' do
      subject.stub(:raw_info).and_return({ 'username' => 'me', 'avatar_url' => 'http://example.com' })
      subject.info[:name].should == 'me'
  ...
  context "#raw_info" do
    it "should use info api" do
      access_token.should_receive(:get).with('api/v1/me.json').and_return(response)
